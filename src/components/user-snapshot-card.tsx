...
import type { UserSnapshotMetadata } from '@/lib/database.types';
...
// The rich attachment "New signup" and "Account deleted" admin notifications
// render instead of their plain body text -- same slot RankedPosterRow
// occupies for multi-story notifications. Purely informational (no actions):
// Premium is still granted by hand from Admin -> Users, this just saves a
// trip to look the user up.
export function UserSnapshotCard({ metadata }: { metadata: UserSnapshotMetadata }) {
  const name = metadata.display_name || (metadata.type === 'account_deleted' ? 'A former user' : 'A new user');
